...
Read `docs/devlog/summary.md` and `docs/architecture/ARCHITECTURE.md` (or `docs/ARCHITECTURE.md` if that's where it lives) before starting work. Neither exists yet — this repo is pre-code (no `package.json`, `src/`, or `docs/` tree); until the app lands, `README.md` is the only project context.
...
- Verify every change against this project's gates: `npm test`, `npx tsc --noEmit`, `npm run build`. All three must pass before declaring a task done. (Pre-code caveat: no `package.json` exists yet, so these gates cannot run today — they are the intended gate set. Wire them up with the first code commit and correct this list to match the real scripts.)
...
When you do dispatch, the team roles below describe how to brief them. The Team Lead role is always you (the main agent). When a subagent cannot read project instructions on its own, paste the relevant rules from this file into its prompt.
...
- **Software engineer**: Handles code writing.
  - After coding, ask the code reviewer to review (see Code review section) and iterate. Multi-CLI reviews run in the background; timing and poller mechanics are in the fleet-canonical runbook `../loop-ops/docs/skills/multi-cli-review.md` (this repo's `.claude/skills/multi-cli-review/SKILL.md` is a thin stub for repo-specific notes).
  ...
  - Save reviewer synthesis under `docs/threads/current/<objective>/<date>/<iteration_number>/`. The `<objective>` folder is a concise kebab-case phrase naming the work; for full-codebase reviews, use `full`.
  ...
  - After folding the final iteration's `REVIEW.md` into the devlog entry for the task, move the objective folder from `docs/threads/current/` to `docs/threads/done/`. The done thread stays as a historical artifact (do not delete).
...
The default adversarial pass for non-trivial work is the in-process Workflow (see Core rules). Run the multi-CLI review (Codex + Claude, each reviewing independently) on high-risk changes and full-codebase audits. All multi-CLI mechanics — current review model pins, exact commands, sandbox flags, the background-run/poller pattern, the Codex output-extraction recipe, and CLI failure modes — live in the fleet-canonical runbook `../loop-ops/docs/skills/multi-cli-review.md` (review pins bump there, once for the whole fleet); read it before every multi-CLI session, then this repo's thin stub `.claude/skills/multi-cli-review/SKILL.md` for repo-specific notes.
...
- `docs/architecture/ARCHITECTURE.md` (or `docs/ARCHITECTURE.md`) — Component Map row + Boundaries paragraph for the new subsystem; lifecycle / data-flow ASCII updated if the flow changes.
...
- `docs/learning/lessons.md` — when you encounter a non-obvious failure mode worth preserving for future sessions (a recurring trap, a rule that prevented a reasonable-seeming mistake, a process step that turned out load-bearing). **Each lesson MUST start with this evidence-anchor table** — without anchors a "lesson" is folklore and self-improvement becomes prompt drift:
  ...
  | Test added | exact test node id — e.g. `tests/path/test.spec.ts::name` (or `n/a — process lesson` for review/tooling-only lessons) |
  | Behavior delta | concrete before/after — what would have happened in production without the fix; for ingestion/pipeline changes include the dataset / run / index ID and the data-quality metric that moved |
...
- Respect the boundaries documented in `docs/architecture/ARCHITECTURE.md` (or `docs/ARCHITECTURE.md`). If a boundary seems wrong, flag it instead of silently violating it.
- If architecture changes, update the relevant sections, append a row to the drift log, and mention the update in the devlog.
- Do not update the architecture doc for non-structural fixes, refactors, UI tweaks, or test-only work.
- Never delete a Key Architectural Decision; add a newer decision that supersedes it.
