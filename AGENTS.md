...
A news portal that curates, checks facts, keeps articles organized, and draws inspiration from them.

Pre-code: no app, `package.json`, `src/`, or `docs/` exist yet — `README.md` is the only project context, and the stray `node_modules/` is an orphan from an abandoned scaffold, not evidence of a build.
...
No gates yet — establish them with the first code.
